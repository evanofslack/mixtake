...
export default function Player(): JSX.Element {
    ...
    return (
        <div>
            ...
            {!loading && state && (
                <div className="bg-light-secondary flex flex-col items-center rounded-md px-6 py-3 text-white">
                    <h1>Currently listening to</h1>
                    <p>{state.item.album.name}</p>
                    {state.item.album.artists && <p>by {state.item.album.artists[0].name}</p>}
